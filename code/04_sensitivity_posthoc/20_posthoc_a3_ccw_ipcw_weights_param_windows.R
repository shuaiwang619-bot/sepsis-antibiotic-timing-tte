# post-hoc A3 CCW IPCW estimation with parameterized treatment windows
# Scope: estimate arm-specific stabilized inverse probability of artificial
# censoring weights for .imp = 1-5 under alternative antibiotic windows.
#
# Main IPCW model:
...
window_spec <- if (length(args) >= 2) args[2] else "0,3,6,12"
write_time_weight_output <- if (length(args) >= 3) {
  tolower(args[3]) %in% c("true", "t", "1", "yes", "y", "write")
} else {
  FALSE
}

parse_window_spec <- function(x) {
  bounds <- suppressWarnings(as.numeric(strsplit(x, ",", fixed = TRUE)[[1]]))
  if (length(bounds) != 4L || any(!is.finite(bounds)) || bounds[1] != 0 ||
      any(diff(bounds) <= 0) || bounds[4] > 12) {
    stop("window_spec must be four increasing comma-separated hour boundaries from 0 through <=12, e.g. 0,2,6,12.", call. = FALSE)
  }
  bounds
}

format_hour <- function(x) {
  ifelse(abs(x - round(x)) < 1e-8, as.character(as.integer(round(x))), format(x, trim = TRUE, scientific = FALSE))
}

window_bounds <- parse_window_spec(window_spec)
window_starts <- window_bounds[1:3]
window_ends <- window_bounds[2:4]
window_labels <- paste0(format_hour(window_starts), "-", format_hour(window_ends), "h")
late_window_label <- paste0(">", format_hour(window_bounds[4]), "h")
...
medical_delay_window <- function(delay) {
  ...
  for (jj in seq_along(window_labels)) {
    start_h <- window_starts[jj]
    end_h <- window_ends[jj]
    if (jj == 1L) {
      idx <- !is.na(delay) & delay >= start_h & delay <= end_h
    } else {
      idx <- !is.na(delay) & delay > start_h & delay <= end_h
    }
    out[idx] <- window_labels[jj]
  }
  out[!is.na(delay) & delay > window_bounds[4] & delay <= 72] <- late_window_label
  out
}
...
censor_hour <- function(arm, delay) {
  ...
  for (jj in seq_along(window_labels)) {
    ...
    i_early <- arm == aa & start_h > 0 & !is.na(delay) & delay <= start_h
    out[i_early] <- pmax(0, ceiling(delay[i_early]))

    i_late <- arm == aa & (is.na(delay) | delay > end_h)
    out[i_late] <- end_h
  }
...
censor_reason <- function(arm, delay) {
  ...
  for (jj in seq_along(window_labels)) {
    ...
    out[arm == aa & start_h > 0 & !is.na(delay) & delay <= start_h] <- "early_init"
    out[arm == aa & (is.na(delay) | delay > end_h)] <- "late_or_no_init"
  }
...
cat("post-hoc A3 CCW IPCW weights\n")
cat("Run time:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("Window spec:", window_spec, "\n")
cat("Arms:", paste(window_labels, collapse = ", "), "\n")
...
boundary_check <- eligible_wide[ccw_role_factor == "analyzable", .(
  analyzable = .N,
  original_window_mismatch = sum(abx_window_factor != delay_window_medical, na.rm = TRUE),
  missing_delay_window = sum(is.na(delay_window_medical))
), by = .imp][order(.imp)]
boundary_check[, window_spec := window_spec]
if (boundary_check[, any(missing_delay_window > 0)]) {
  print(boundary_check)
  stop("Custom antibiotic windows do not cover all analyzable antibiotic delays.", call. = FALSE)
}

arm_def <- data.table(
  arm = window_labels,
  arm_id = seq_along(window_labels),
  arm_start_h = window_starts,
  arm_end_h = window_ends
)
