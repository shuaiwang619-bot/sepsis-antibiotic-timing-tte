# post-hoc A3 CCW weighted pooled logistic model with dynamic covariate set
# Scope: alternative antibiotic-window analyses where arm labels differ from
# the primary 0-3/3-6/6-12 h windows.
...
cat("post-hoc A3 CCW dynamic-covariate pooled logistic model\n")
cat("Run time:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("Analysis label:", analysis_label, "\n")
cat("Person-period table:", pp_file, "\n")

need_cols <- c(
  ".imp", "clone_id", "stay_id", "arm", "arm_id", "time_axis", "event_30d_period",
  "ipcw_stabilized",
...
pp[, `:=`(
  ...
  arm_id = as.integer(as_num(arm_id)),
...
arm_levels <- pp[, unique(as.character(arm[order(arm_id)]))]
if (length(arm_levels) != 3L) {
  stop("Expected exactly three treatment arms in person-period table; found: ", paste(arm_levels, collapse = ", "), call. = FALSE)
}
arm_ref <- arm_levels[1]
arm_mid <- arm_levels[2]
arm_late <- arm_levels[3]
coef_mid <- paste0("arm", arm_mid)
coef_late <- paste0("arm", arm_late)
contrast_mid_ref <- paste0(arm_mid, "_vs_", arm_ref)
contrast_late_ref <- paste0(arm_late, "_vs_", arm_ref)
contrast_late_mid <- paste0(arm_late, "_vs_", arm_mid)
...
for (ii in sort(unique(pp$.imp))) {
  ...
  strategy_variance_ok <- all(
    c(coef_mid, coef_late) %in% names(beta),
    is.finite(var_diag[match(c(coef_mid, coef_late), names(beta))]),
    var_diag[match(c(coef_mid, coef_late), names(beta))] > 0
  )
  ...
  if (all(c(coef_mid, coef_late) %in% names(beta))) {
    l_mid <- rep(0, length(beta))
    names(l_mid) <- names(beta)
    l_mid[coef_mid] <- 1

    l_late <- rep(0, length(beta))
    names(l_late) <- names(beta)
    l_late[coef_late] <- 1

    l_late_vs_mid <- l_late - l_mid
    ...
    names(contrast_specs) <- c(contrast_mid_ref, contrast_late_ref, contrast_late_mid)
  ...
  risk_i[, .imp := ii]
  ref <- risk_i[arm == arm_ref, cumulative_risk_30d]
  risk_i[, reference_arm := arm_ref]
  risk_i[, risk_difference_vs_reference := cumulative_risk_30d - ref]
  risk_i[, risk_ratio_vs_reference := cumulative_risk_30d / ref]
...
risk_summary <- risk_long[, .(
  ...
  reference_arm = first(reference_arm),
  risk_difference_vs_reference = mean(risk_difference_vs_reference),
  risk_ratio_vs_reference = mean(risk_ratio_vs_reference)
...
cat("=== post-hoc A3 CCW dynamic-covariate weighted pooled logistic model ===\n", file = out_report)
cat("analysis_label:", analysis_label, "\n", file = out_report, append = TRUE)
cat("arm_levels:", paste(arm_levels, collapse = ", "), "\n", file = out_report, append = TRUE)
cat("input:", pp_file, "\n", file = out_report, append = TRUE)
