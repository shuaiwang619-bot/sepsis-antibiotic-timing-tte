# post-hoc A3 CCW covariate balance diagnostics
# Scope: baseline covariate SMD before and after IPCW among strategy-adherent
# clones under alternative antibiotic windows.
...
compute_balance <- function(dat, variable_spec, weight_col, weighting, sample_label) {
  arm_levels <- dat[, unique(as.character(arm[order(arm_id)]))]
  if (length(arm_levels) != 3L) {
    stop("Expected exactly three treatment arms for balance diagnostics; found: ", paste(arm_levels, collapse = ", "), call. = FALSE)
  }
  contrast_def <- data.table(
    contrast = c(
      paste0(arm_levels[2], "_vs_", arm_levels[1]),
      paste0(arm_levels[3], "_vs_", arm_levels[1]),
      paste0(arm_levels[3], "_vs_", arm_levels[2])
    ),
    arm_ref = c(arm_levels[1], arm_levels[1], arm_levels[2]),
    arm_comp = c(arm_levels[2], arm_levels[3], arm_levels[3])
  )
...
cat("post-hoc A3 CCW covariate balance diagnostics\n")
...
if (requireNamespace("ggplot2", quietly = TRUE)) {
  ...
  p <- ggplot(plot_dat, aes(x = max_abs_smd, y = display_label, color = weighting_label)) +
    ...
    labs(
      ...
      y = NULL,
      color = NULL,
      title = "Post-hoc A3 baseline covariate balance"
    ) +
