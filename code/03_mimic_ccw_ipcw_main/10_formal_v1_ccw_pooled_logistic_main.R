# formal_v1 CCW main 30-day weighted pooled logistic model
# Scope: fit the main marginal structural outcome model across .imp = 1-5.
#
# Outcome model:
#   event_30d_period ~ arm + time_axis + baseline covariates
#
# Baseline covariates:
#   demographics, Charlson, baseline SOFA, baseline shock, and missing flags.
#   meanBP is intentionally excluded from the main model.
...
cat("formal_v1 CCW main pooled logistic model\n")
...
model_cols <- c(
  "event_30d_period", "ipcw_stabilized", "arm", "time_axis",
  "age_at_icu_model", "sex_factor", "race_group", "admission_type_group",
  "charlson_comorbidity_index_model",
...
if (model_na[, any(rowSums(.SD) > 0), .SDcols = model_cols]) {
  print(model_na)
  stop("Main outcome model columns contain NA.", call. = FALSE)
}
...
cat("=== formal_v1 CCW main weighted pooled logistic model ===\n", file = out_report)
...
cat("meanBP is excluded from the main model by design.\n\n", file = out_report, append = TRUE)
...
cat("Risks are model-standardized over the imputed eligible cohort baseline covariate distribution.\n", file = out_report, append = TRUE)
cat("Risk uncertainty is not computed in this main script; point risks are averaged across imputations.\n", file = out_report, append = TRUE)
cat("Done\n", file = out_report, append = TRUE)
...
cat("PASS: main pooled logistic model completed.\n")
